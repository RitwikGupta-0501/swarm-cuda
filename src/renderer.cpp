...
static_assert(sizeof(CameraUboStd140) % 16 == 0, "UBO must be 16-byte aligned.");
...
bool Renderer::init(const RendererConfig& cfg, int viewportW, int viewportH,
  ...
  if (!mDebug->init(outError)) return false;
  mDebug->resize(viewportW, viewportH);

  if (!createCameraUbo(outError))          return false;
  if (!createAgentPipeline(outError))      return false;
  if (!createSharedAgentBuffer(outError))  return false;
  if (!createDebugVecPipeline(outError))   return false;  // Task 1.8
  if (!createCullPipeline(outError))       return false;  // Task 1.7
  if (mCfg.trailLength > 0) {
    if (!createTrailPipelines(outError))   return false;  // Task 1.6
  }
  if (!createAgentTypeSsbo(outError))      return false;  // Prey/Predator type
  if (!createBloomPipeline(outError))      return false;  // Glow/Bloom
  rebuildBloomTargets();
  if (!registerCudaInterop(outError))      return false;
  return true;
}
...
void Renderer::resize(int viewportW, int viewportH) {
  ...
  mViewportH = viewportH;
  mCamera.setViewport(viewportW, viewportH);
  if (mDebug) mDebug->resize(viewportW, viewportH);
  rebuildBloomTargets();
}

void Renderer::setCameraMode(CameraMode mode) {
  mCamera.setMode(mode);
}
...
void Renderer::destroyGl() {
  // Original resources.
  if (mAgentProgram)  glDeleteProgram(mAgentProgram);
  if (mPointsProgram) glDeleteProgram(mPointsProgram);
  if (mAgentVao)      glDeleteVertexArrays(1, &mAgentVao);
  if (mAgentBaseVbo)  glDeleteBuffers(1, &mAgentBaseVbo);
  if (mAgentVbo)      glDeleteBuffers(1, &mAgentVbo);
  if (mCameraUbo)     glDeleteBuffers(1, &mCameraUbo);

  // Task 1.8 — debug vectors.
  if (mDebugVecProgram) glDeleteProgram(mDebugVecProgram);
  // mAgentSsbo is an alias binding — no separate buffer to delete.
  if (mDummyVao) glDeleteVertexArrays(1, &mDummyVao);

  // Agent type SSBO.
  if (mAgentTypeSsbo) glDeleteBuffers(1, &mAgentTypeSsbo);

  // Task 1.7 — culling.
  if (mCullProgram)      glDeleteProgram(mCullProgram);
  if (mVisibleAgentSsbo) glDeleteBuffers(1, &mVisibleAgentSsbo);
  if (mDrawCmdBuf)       glDeleteBuffers(1, &mDrawCmdBuf);
  if (mCullAgentVao)     glDeleteVertexArrays(1, &mCullAgentVao);
  if (mCullAgentBaseVbo) glDeleteBuffers(1, &mCullAgentBaseVbo);

  // Task 1.6 — trails.
  if (mTrailUpdateProgram) glDeleteProgram(mTrailUpdateProgram);
  if (mTrailRenderProgram) glDeleteProgram(mTrailRenderProgram);
  if (mTrailSsbo)          glDeleteBuffers(1, &mTrailSsbo);
  ...
  if (mVisibleAgentTypeSsbo) glDeleteBuffers(1, &mVisibleAgentTypeSsbo);
  if (mVisibleAgentIdSsbo)   glDeleteBuffers(1, &mVisibleAgentIdSsbo);
  if (mTrailDrawCmdBuf)      glDeleteBuffers(1, &mTrailDrawCmdBuf);
  mVisibleAgentTypeSsbo = 0;
  mVisibleAgentIdSsbo   = 0;
  mTrailDrawCmdBuf      = 0;

  // Bloom cleanup.
  if (mBloomBrightProgram)    glDeleteProgram(mBloomBrightProgram);
  if (mBloomBlurProgram)      glDeleteProgram(mBloomBlurProgram);
  if (mBloomCompositeProgram) glDeleteProgram(mBloomCompositeProgram);
  destroyBloomTargets();
  ...
  if (mInterop.cudaGraphicsResource) {
    cudaUnregisterAgentBuffer(
        reinterpret_cast<cudaGraphicsResource*>(mInterop.cudaGraphicsResource));
...
bool Renderer::createCameraUbo(std::string* outError) {
  ...
  if (!mCameraUbo) {
    if (outError) *outError = "Failed to create Camera UBO.";
    return false;
  }
  SWARM_GL_CALL(glNamedBufferData(mCameraUbo, sizeof(CameraUboStd140), nullptr, GL_DYNAMIC_DRAW));
...
void Renderer::updateCameraUbo(float timeSeconds) {
  ...
  u.viewport_Zoom_Mode = glm::vec4(
      static_cast<float>(mViewportW),
      static_cast<float>(mViewportH),
      cam.zoom,
      static_cast<float>(static_cast<int>(cam.mode)));
  SWARM_GL_CALL(glNamedBufferSubData(mCameraUbo, 0, sizeof(CameraUboStd140), &u));
}
...
bool Renderer::createAgentPipeline(std::string* outError) {
  ...
  SWARM_GL_CALL(glNamedBufferData(mAgentBaseVbo, sizeof(base), base, GL_STATIC_DRAW));

  // Binding 0: base geometry (vec2).
  SWARM_GL_CALL(glVertexArrayVertexBuffer(mAgentVao, 0, mAgentBaseVbo, 0, sizeof(float) * 2));
  SWARM_GL_CALL(glEnableVertexArrayAttrib(mAgentVao, 0));
  SWARM_GL_CALL(glVertexArrayAttribFormat(mAgentVao, 0, 2, GL_FLOAT, GL_FALSE, 0));
...
bool Renderer::createSharedAgentBuffer(std::string* outError) {
  ...
  if (!mAgentVbo) {
    if (outError) *outError = "Failed to create shared Agent VBO.";
    return false;
  }
...
bool Renderer::registerCudaInterop(std::string* outError) {
  ...
  if (!cudaRegisterAgentBuffer(mAgentVbo, &res, outError)) return false;
...
bool Renderer::createDebugVecPipeline(std::string* outError) {
  ShaderProgram p;
  if (!p.loadFromFiles("shaders/debug_vec.vert", "shaders/debug_vec.frag", outError))
    return false;
...
bool Renderer::createCullPipeline(std::string* outError) {
  ShaderProgram comp;
  if (!comp.loadCompute("shaders/cull.comp", outError)) return false;
  ...
  const DrawArraysIndirectCommand initCmd{3, 0, 0, 0}; // vertCount=3, instanceCount=0
  SWARM_GL_CALL(glCreateBuffers(1, &mDrawCmdBuf));
  SWARM_GL_CALL(glNamedBufferData(mDrawCmdBuf, sizeof(DrawArraysIndirectCommand),
  ...
  const GLsizeiptr typeBytes = static_cast<GLsizeiptr>(mCfg.maxAgents) * sizeof(uint32_t);
  ...
  // Trail indirect command buffer: { vertCount, instanceCount, first, baseInstance }
  // vertCount = trailLength, instances = current surviving agents
  const DrawArraysIndirectCommand trailCmd{ static_cast<uint32_t>(mCfg.trailLength), 0, 0, 0 };
  SWARM_GL_CALL(glCreateBuffers(1, &mTrailDrawCmdBuf));
  SWARM_GL_CALL(glNamedBufferData(mTrailDrawCmdBuf, sizeof(DrawArraysIndirectCommand),
                                   &trailCmd, GL_DYNAMIC_DRAW));

  SWARM_GL_CALL(glCreateVertexArrays(1, &mCullAgentVao));

  // Binding 0: base geometry.
  SWARM_GL_CALL(glVertexArrayVertexBuffer(mCullAgentVao, 0, mCullAgentBaseVbo, 0, sizeof(float) * 2));
  SWARM_GL_CALL(glEnableVertexArrayAttrib(mCullAgentVao, 0));
  SWARM_GL_CALL(glVertexArrayAttribFormat(mCullAgentVao, 0, 2, GL_FLOAT, GL_FALSE, 0));
...
void Renderer::dispatchCull(int agentCount, const CameraMatrices& cam) {
  ...
  glUniform2f(glGetUniformLocation(mCullProgram, "uCullMin"), aabb.min.x, aabb.min.y);
  glUniform2f(glGetUniformLocation(mCullProgram, "uCullMax"), aabb.max.x, aabb.max.y);
...
bool Renderer::createTrailPipelines(std::string* outError) {
  ShaderProgram upd;
  if (!upd.loadCompute("shaders/trail_update.comp", outError)) return false;
  mTrailUpdateProgram = upd.release();

  ShaderProgram rend;
  if (!rend.loadFromFiles("shaders/trail.vert", "shaders/trail.frag", outError)) return false;
  mTrailRenderProgram = rend.release();

  // Trail ring-buffer: maxAgents × trailLength × 2 floats.
  const GLsizeiptr trailBytes =
      static_cast<GLsizeiptr>(mCfg.maxAgents) *
...
void Renderer::dispatchTrailUpdate(int agentCount) {
  ...
  glUniform1ui(glGetUniformLocation(mTrailUpdateProgram, "uFrameIdx"), mTrailFrameIdx);
...
void Renderer::renderTrails(int agentCount) {
  ...
  glUniform1i(glGetUniformLocation(mTrailRenderProgram, "uCullingEnabled"), useCulling ? 1 : 0);
...
// Render Entry Points
// =============================================================================
void Renderer::render(int agentCount, float timeSeconds, const FrameStats& frameStats) {
  agentCount = std::max(0, std::min(agentCount, mCfg.maxAgents));

  mCamera.update(frameStats.dtSeconds > 0.0 ? static_cast<float>(frameStats.dtSeconds) : 0.016f);
  ...
  const bool usePoints = (cam.mode == CameraMode::Perspective25D) ? false : (cam.zoom < 0.2f);
  const GLuint prog = usePoints ? mPointsProgram : mAgentProgram;
  glUseProgram(prog);
  glUniform1i(glGetUniformLocation(prog, "uMode"),   static_cast<int>(mVizMode));
  ...
  if (mShowVelocityVectors && mDebugVecProgram) {
    glPushDebugGroup(GL_DEBUG_SOURCE_APPLICATION, 6, -1, "debug_velocity_vectors");
    glUseProgram(mDebugVecProgram);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, mAgentVbo);
    glBindVertexArray(mDummyVao);  // required by GL core profile even with SSBO-only shaders

    const float vecScale = 3.0f; // tune: world-units per unit of velocity
    glUniform1f(glGetUniformLocation(mDebugVecProgram, "uVecScale"), vecScale);
    glDisable(GL_DEPTH_TEST);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
bool Renderer::createAgentTypeSsbo(std::string* outError) {
  ...
  SWARM_GL_CALL(glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 5, mAgentTypeSsbo));
  return true;
}

void Renderer::uploadAgentTypes(const uint32_t* types, int count) {
  if (!mAgentTypeSsbo || !types || count <= 0) return;
  const int safeCount = std::min(count, mCfg.maxAgents);
  SWARM_GL_CALL(glNamedBufferSubData(
      mAgentTypeSsbo, 0, safeCount * sizeof(uint32_t), types));
}
...
bool Renderer::createBloomPipeline(std::string* outError) {
  ...
  if (!p1.loadFromFiles("shaders/fullscreen.vert", "shaders/bloom_bright.frag", outError)) return false;
  mBloomBrightProgram = p1.release();

  if (!p2.loadFromFiles("shaders/fullscreen.vert", "shaders/bloom_blur.frag", outError)) return false;
  mBloomBlurProgram = p2.release();

  if (!p3.loadFromFiles("shaders/fullscreen.vert", "shaders/bloom_composite.frag", outError)) return false;
...
void Renderer::destroyBloomTargets() {
  if (mBloom.sceneFbo) glDeleteFramebuffers(1, &mBloom.sceneFbo);
  if (mBloom.colorTex) glDeleteTextures(1, &mBloom.colorTex);
  if (mBloom.depthRbo) glDeleteRenderbuffers(1, &mBloom.depthRbo);

  if (mBloom.brightFbo) glDeleteFramebuffers(1, &mBloom.brightFbo);
  if (mBloom.brightTex) glDeleteTextures(1, &mBloom.brightTex);

  if (mBloom.pingFbo[0]) glDeleteFramebuffers(2, mBloom.pingFbo);
  if (mBloom.pingTex[0]) glDeleteTextures(2, mBloom.pingTex);
...
void Renderer::rebuildBloomTargets() {
  ...
  if (mViewportW <= 0 || mViewportH <= 0) return;
  ...
  glNamedRenderbufferStorage(mBloom.depthRbo, GL_DEPTH_COMPONENT32F, mViewportW, mViewportH);

  glNamedFramebufferTexture(mBloom.sceneFbo, GL_COLOR_ATTACHMENT0, mBloom.colorTex, 0);
  glNamedFramebufferRenderbuffer(mBloom.sceneFbo, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, mBloom.depthRbo);
  ...
  glNamedFramebufferTexture(mBloom.brightFbo, GL_COLOR_ATTACHMENT0, mBloom.brightTex, 0);
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    glTextureParameteri(mBloom.pingTex[i], GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTextureParameteri(mBloom.pingTex[i], GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glNamedFramebufferTexture(mBloom.pingFbo[i], GL_COLOR_ATTACHMENT0, mBloom.pingTex[i], 0);
  }
}
...
void Renderer::runBloomPasses() {
  ...
  glBindTextureUnit(0, mBloom.colorTex);
  glBindTextureUnit(1, mBloom.pingTex[horizontal ? 0 : 1]); // the last buffer written
  ...
  glUniform1f(glGetUniformLocation(mBloomCompositeProgram, "uBloomStrength"), 0.5f);
  glDrawArrays(GL_TRIANGLES, 0, 3);
}
